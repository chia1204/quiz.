...
function setup() {
  ...
  for (let i = 0; i < 50; i++) {
    let x = random(width);
    let y = random(height);
    let size = random(10, 50); // 隨機大小
    let circleColor = color(random(100, 200), random(100, 200), 255); // 藍色系
    circles.push({ x, y, size, circleColor });
  }
}
